"""Tool registration, validation, and dispatch."""
...
class ToolSpec:
    ...
    @property
    def name(self) -> str:
        """Return the action-shaped tool name."""
...
class ToolRegistry:
    ...
    def __init__(self, specs: Iterable[ToolSpec]) -> None:
        ...
        for spec in specs:
            ...
            if not isinstance(parameters, dict):
                raise ValueError(
                    f"Tool {name} must define an object parameter schema"
                )
    ...
    def schemas(self) -> tuple[ToolSchema, ...]:
        """Return schemas that can be supplied to the model."""
    ...
    def requires_approval(self, tool_name: str) -> bool:
        """Return whether a registered tool requires human approval."""
    ...
    def execute(self, tool_call: ToolCall) -> ToolOutput:
        ...
        validator = self._validators[tool_call.name]
        errors = sorted(
    ...
    def _validate_output(
        *,
        ...
        """Reject malformed tool output before it reaches the model."""
        ...
        if not isinstance(ok, bool):
            raise ToolOutputError(
                f"Tool {tool_name} result must contain boolean 'ok'"
            )

        if ok and "data" not in output:
            raise ToolOutputError(
                f"Successful tool {tool_name} result must contain 'data'"
            )

        if not ok and "error" not in output:
            raise ToolOutputError(
                f"Failed tool {tool_name} result must contain 'error'"
            )
    ...
    def _error(
        *,
        ...
        """Build a structured registry-level error."""
...
def create_editorial_registry(
    ...
    return ToolRegistry(
        (
            ToolSpec(
                schema=READ_PRESS_RELEASE_SCHEMA,
                handler=partial(
                    read_press_release,
                    store,
                ),
            ...
            ToolSpec(
                schema=SAVE_LINKEDIN_DRAFT_SCHEMA,
                handler=partial(
                    save_linkedin_draft,
                    store,
                ),
            ...
            ToolSpec(
                schema=READ_LINKEDIN_DRAFT_SCHEMA,
                handler=partial(
                    read_linkedin_draft,
                    store,
                ),
