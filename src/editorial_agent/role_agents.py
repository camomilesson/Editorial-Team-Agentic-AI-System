        ...
        *,
        ...
        for step in range(1, self.max_steps + 1):
            try:
                ...
                raise RoleAgentError("Model could not produce a role response.") from exc
            ...
            if not response.tool_calls:
                try:
                    ...
                    raise RoleAgentError("Model returned invalid structured output.") from exc
            ...
            for tool_call in response.tool_calls:
                ...
                try:
                    ...
                    raise RoleAgentError("Role retrieval tool failed.") from exc
                if on_tool_completed is not None:
                    on_tool_completed(self.role, tool_call, result)
                if result.get("ok") is not True:
                    raise RoleAgentError("Role retrieval tool returned an error.")
                results.append(
                    ToolResult(
        ...
        raise RoleAgentError("Role exceeded its maximum model-step budget.")
